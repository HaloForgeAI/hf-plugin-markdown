...
const setPopoverPosition = (vditor: IVditor, element: HTMLElement) => {
    ...
    if (element.getAttribute("data-type") === "code-block") {
        const top = targetElement.offsetTop + Math.max(0, targetElement.offsetHeight - vditor.wysiwyg.popover.clientHeight - 8);
        const left = targetElement.offsetLeft + Math.max(0, targetElement.offsetWidth - vditor.wysiwyg.popover.clientWidth - 10);
        vditor.wysiwyg.popover.style.top = Math.max(0, top - vditor.wysiwyg.element.scrollTop) + "px";
        ...
        vditor.wysiwyg.popover.setAttribute("data-top", top.toString());
        return;
    }
