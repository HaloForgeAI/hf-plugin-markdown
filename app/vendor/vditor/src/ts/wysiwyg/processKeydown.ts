...
export const processKeydown = (vditor: IVditor, event: KeyboardEvent) => {
    ...
    if (codeRenderElement) {
        ...
        if (event.key === "Escape" && codeRenderElement.children.length === 2) {
            ...
            (codeRenderElement.lastElementChild as HTMLElement).style.display = "";
        ...
        if (!isCtrl(event) && !event.shiftKey && event.altKey && event.key === "Enter" &&
            ...
            const inputElement =
                (codeRenderElement.querySelector(".hf-code-language-input") ||
                    vditor.wysiwyg.popover.querySelector(".vditor-input")) as HTMLInputElement | null;
            if (inputElement) {
                inputElement.focus();
                inputElement.select();
                event.preventDefault();
